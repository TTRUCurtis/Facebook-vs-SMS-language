...
#    options["write_to_csv"] = str(str(Path.home()) + "/pii_tagged_removed_data_"+dt.datetime.now().strftime("%Y%m%d%H%M%S")+".csv")
    ...
    while (((options["batches_complete"] * options["batch_fetch"]) < options["fetch_count"]) and data_remaining):
        ...
        with engine.connect() as connection:
            ...
            query = db.select([orig_data]).offset(options["batches_complete"] * options["batch_fetch"]).limit(options["batch_fetch"])
...
#        print(result_set[0:10])
        ...
        # Put Result into PANDAS dataframe
            ...
            orig_df.insert(3, "scrubbed_text", "", True )    
            ...
            if ((len(orig_df) == 0) and (options["batches_complete"] == 0)):
                ...
                print("Processing Batch:"+str(options["batches_complete"]))
                ...
                if (options["commit_to_db"]):
                    #FBCHANGE - NO ID COLUMN
                    temp_df = orig_df[["user_id", "message_id", "message", "scrubbed_text", "place_id", "place_name","link",
                                       "created_time","inserted_on","privacy_value","privacy_desc","privacy_friends","survey_id",
                                       "insert_stamp","update_stamp","label","is_complete", "has_changed", "change_segments_text"]]
        ...
        #       Create the "scrubbed" data table with just the complete messages with PII removed.
         #FBCHANGE - NO ID COLUMN
                scrubbed_df = orig_df.loc[orig_df["is_complete"]==1, ["user_id", "message_id", "scrubbed_text","place_id", "place_name","link",
                                       "created_time","inserted_on","privacy_value","privacy_desc","privacy_friends","survey_id",
                                       "insert_stamp","update_stamp","label"]]
                if (options["commit_to_db"]):
...
def check_for_input_change(df, current_row, prev_row):
    ...
    # For FB Data, every row is different:
    df.at[prev_row, "is_complete"] = 1
    return
...
def remove_brackets(df,row_id):
    ...
    # FB Data doesn't have surrounding brackets
    
    df.at[row_id, "scrubbed_text"] = df.at[row_id, "message"]
    return
...
#    doc = nlp(df.at[row_id, "scrubbed_text"])
    ...
    #FBCHANGE - DON"T SCRUB NULL ITEMS
    if (df.at[row_id, "scrubbed_text"] is None):
        return
...
def do_token_change_work(df, row_id):
    ...
    end_char = 0
    data_row = row_id
    # FBCHANGE - Every row is different:
    if (not (df.at[row_id - 1, "scrubbed_text"] is None)):
